...
% P_centro=[0 0 -0.255];
P_centro=[0 0 -(N_espiras-1)*raio_cabo-0.255];  %raio_cabo];% % para a bobine ficar à face do plano!
plano_paralelo=1;
...
corte_em_z=0; % altura a que queremos colocar o plano horizontal
    %----------------------------------------------------------------------
    %-------------------------------CIRCULO---------------------------------
    %----------------------------------------------------------------------
...
%     
    ...
    axis on;
    surf(X,Y,B)
    title('Campo magnético (Configuração 1)')
    xlabel('X');
    ylabel('Y');
    zlabel('Campo magnético [Tesla]');
    %------------------------------------
    figure
    axis on;
    plot(resultados_xy(:,7)*1e6,'gx-');   %----------ATENCAO AO /8
    grid on;
    title('Campo magnético nos diversos pontos - plano xy (Configuração 1)')
    xlabel('Indice do ponto');
    ylabel('Campo magnético [microTesla]');
